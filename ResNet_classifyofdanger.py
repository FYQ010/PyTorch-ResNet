...
def save_plots(train_acc, valid_acc, train_loss, valid_loss, model_name="", optimizer='', save_path='outputs'):
    ...
    plt.plot(
        train_loss, color='m', linestyle='-',
        label='train loss'
    )
    plt.plot(
        valid_loss, color='crimson', linestyle='-',
        label='validataion loss'
    )

    fig.suptitle('model:'+model_name+" "+optimizer, fontsize=16)
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'{save_path}/loss—trend.png')
    
    # accuracy
    fig = plt.figure(figsize=(7, 5))
    plt.style.use('seaborn-white')
    plt.plot(
        train_acc, color='cyan', linestyle='-',
        label='train accuracy'
    )
    plt.plot(
        valid_acc, color='lime', linestyle='-',
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(f'{save_path}/accuracy-trend.png')
...
for class_path in classes:
    ...
    targets += [class2label[class_path]] * len(os.listdir(path))  #labels = 0 or 1 or 2
...
def train(model, train_loader, optimizer, criterion):
    ...
    assert len(train_loader) == counter, f"{len(train_loader)}, {counter}"
...
def validate(model, test_loader, criterion):
    ...
    assert len(test_loader) == counter, f"{len(test_loader)}, {counter}"
